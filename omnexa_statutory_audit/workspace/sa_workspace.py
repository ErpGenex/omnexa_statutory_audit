...
"""Full Audit workspace — SAP SD / Van Sales parity catalog."""
...
WORKSPACE_NAME = "Audit"

_SHORTCUT_COLORS = ("Blue", "Green", "Orange", "Red", "Cyan", "Purple", "Teal", "Pink", "Yellow")

WORKSPACE_SECTIONS: list[tuple[str, list[WorkspaceLink]]] = [('📊 Dashboards', [('Page', 'sa-executive-dashboard', 'Executive Dashboard'), ('Page', 'sa-servicing-portal', 'Audit Portal')]), ('📋 Engagements', [('DocType', 'Audit Engagement', 'Engagement'), ('DocType', 'Audit Opinion Draft', 'Opinion Draft'), ('DocType', 'Audit Balance Snapshot', 'Balance Snapshot')]), ('🔍 Findings & evidence', [('DocType', 'Audit Finding', 'Finding'), ('DocType', 'Audit Evidence', 'Evidence')]), ('📈 Financial tie-out', [('Report', 'General Ledger', 'General Ledger'), ('Report', 'General Journal', 'General Journal')])]

_REMOVED_SECTIONS = [
	(
		"📊 Dashboards & Mobile",
		[
			("Page", "sa-executive-dashboard", "Executive Dashboard"),
			("Page", "sa-van-sales-pwa", "Van Sales PWA"),
		],
	),
	(
		...
		[
			...
			("DocType", "Audit Vehicle", "Audit Vehicle"),
			("DocType", "Audit Sales Representative", "Sales Representative"),
		],
	),
	(
		"🚚 Field Sales & Distribution",
		[
			("DocType", "Audit Route Plan", "Route Plan"),
			("DocType", "Audit Distribution Order", "Distribution Order"),
			("DocType", "Audit Van Sales Invoice", "Van Sales Invoice"),
			("DocType", "Audit Vehicle Stock Transfer", "Vehicle Stock Transfer"),
		],
	),
	(
		"💰 Commissions & Incentives",
		[
			("DocType", "Audit Commission Rule", "Commission Rule"),
			("DocType", "Audit Commission Settlement", "Commission Settlement"),
		],
	),
	(
		"📋 Tenders & Credit",
		[
			("DocType", "Audit Tender", "Tender"),
			("DocType", "Audit Installment Contract", "Installment Contract"),
		],
	),
	(
		"💳 Finance & ERP",
	...
	(
		...
		[
			("Report", "Audit Sales Summary", "Sales Summary"),
			("Report", "Audit Distribution Fulfillment", "Distribution Fulfillment"),
			("Report", "Audit Vehicle Transfer Summary", "Vehicle Transfer Summary"),
			("Report", "Audit Route Efficiency", "Route Efficiency"),
			("Report", "Audit Rep Target Tracking", "Rep Target Tracking"),
		],
	),
	(
		"📈 Reports · Commissions & Pipeline",
		[
			("Report", "Audit Commission Summary", "Commission Summary"),
			("Report", "Audit Tender Pipeline", "Tender Pipeline"),
			("Report", "Audit Installment Portfolio", "Installment Portfolio"),
		],
	),
	(
		"📈 Reports · Finance & POS",
...
def _build_link_rows() -> list[dict]:
	return build_link_rows_for_app("omnexa_statutory_audit", WORKSPACE_SECTIONS)
...
def _build_content(link_rows: list[dict], ws) -> str:
	...
	content.append(
		{
			"id": "sa-title",
			"type": "header",
			"data": {"text": '<span class="h4"><b>Audit</b></span>', "col": 12},
		}
	)
	section_idx = 0
	link_idx = 0
	for row in link_rows:
		if row.get("type") == "Card Break":
			if section_idx:
				content.append({"id": f"sa-sp-{section_idx}", "type": "spacer", "data": {"col": 12}})
			content.append(
				{
					"id": f"sa-sec-{section_idx}",
					"type": "header",
	...
	if ws.number_cards:
		content.append({"id": "sa-kpi-sp", "type": "spacer", "data": {"col": 12}})
		content.append(
			{
				"id": "sa-kpi-h",
				"type": "header",
	...
	if ws.charts:
		content.append({"id": "sa-ch-sp", "type": "spacer", "data": {"col": 12}})
		content.append(
			{
				"id": "sa-ch-h",
				"type": "header",
...
def sync_sa_workspace_menu(*, save: bool = True, rebuild: bool = True) -> dict:
